...
This addendum supersedes older row-count notes below that still mention `17,046` listings or `2026-06-24` as the freshest runtime data evidence.

- `US-01`: Docker `/monitoring/status` now reports `17,287` listings, `45,764` observations, `23` observation dates, and `last_observed_date=2026-06-26` after a controlled Domclick refresh from `data/raw/domclick/2026-06-26-bulk`.
- `US-02`: descriptive trend evidence is live through `/stats/observation-trend` and the UI/static audit now sees history through `2026-06-26`; period selection, seasonality, and multi-object comparison remain the next analytics upgrade rather than something to overclaim.
- `US-03` and `US-08`: the served selected model remains the validated `random_forest` snapshot trained on `17,046` rows. The `17,287`-listing database does not automatically make a better model; retrain only through the existing candidate/promotion workflow.
- `US-06`: monitoring now exposes bounded `recent_logs` plus compatibility `recent_errors`, and the Domclick scheduler wrapper records pre-batch source failures such as QRATOR into `app_logs`.
- `US-07`: final Docker proof should be recorded through WSL/Docker Compose commands; Windows PowerShell Docker availability is not assumed on this workstation.
...
| US-06 | As an administrator, I want to monitor system status so I can know about problems. Acceptance: source status page, latest ingestion logs, working/not-working indicator, last successful collection time. | Ingestion run accounting, data-quality stats with timestamps, model status, bounded `recent_logs` plus compatibility `recent_errors`, `/monitoring/status`, and Streamlit monitoring section with last successful collection timestamp/source/record count. | Phase 7 / `Partial: status, last-success display, sanitized logs, and Domclick scheduler failure logging implemented` | `IngestionRun` and `AppLog` models in `src/realtyscope/database/models.py`; `GET /stats/data-quality`, `GET /monitoring/status`, and `GET /model/metadata` in `services/api/app/main.py`; `services/streamlit/app.py`; Domclick scheduler `log-error` CLI in `src/realtyscope/ingestion/domclick_scheduled_batch.py`; tests in `tests/test_api_monitoring.py`, `tests/test_streamlit_api_client.py`, `tests/test_streamlit_scaffold.py`, `tests/test_streamlit_ui_payload.py`, and `tests/test_domclick_scheduled_batch.py`. | Scheduler/source failures now have a DB-backed `app_logs` path when the PowerShell wrapper fails before batch ingestion. Monitoring logs are UI-safe and bounded; remaining polish is optional alerting and broader log coverage for unrelated runtime commands. |
