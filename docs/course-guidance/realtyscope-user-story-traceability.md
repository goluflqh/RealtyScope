...
| US-04 | As a user, I want to filter and search data so I can find needed information. Acceptance: sidebar filters, raw data table with pagination, column sorting, text search. | DB-backed listing table via API and Streamlit listing preview with Data Explorer filters. | Phase 7.2 / `Partial: filters implemented` | `/data` and `/listings` support `min_price_rub`, `max_price_rub`, `min_area_m2`, `max_area_m2`, `rooms`, `source_name`, and `search` in `services/api/app/main.py`; Streamlit sidebar controls are wired through `services/streamlit/api_client.py`; tests in `tests/test_api_data_routes.py`, `tests/test_streamlit_api_client.py`, and `tests/test_streamlit_scaffold.py`. | Filter/search acceptance is covered. Remaining polish is clearer table pagination, tab layout, and final browser check after UI polish. |
...
- The remaining Phase 7 user-story gaps are mainly visibility and demo polish: clearer last-update display, runtime log population, table pagination/tabs, demo script, schedule decision, and a fresh final Docker/browser smoke check.
...
- The next strongest user-story slice is clearer monitoring of last successful collection time plus a concise demo runbook.
