...
| Storage: PostgreSQL, Redis Cache, Model Artifacts | PostgreSQL via Docker Compose, SQLAlchemy models, Alembic migrations, Redis service, Redis-backed API data cache, MLflow artifacts, and shared model artifact volume. | PostgreSQL/Alembic/Redis read path `Implemented`; model artifacts `Implemented` for baseline evidence | `docker-compose.yml`; `src/realtyscope/database/models.py`; `alembic/versions/*.py`; `services/mlflow/Dockerfile`; `services/trainer/Dockerfile`; `.env.example`. | Redis now backs the `/listings` and `/data` read path. The baseline model artifact is persisted in Docker-managed volumes. |
| ML Pipeline: Feature Engineering, Model Training, MLflow Registry | Deterministic v1/v2 feature snapshots, non-leaky Ridge baseline, grouped validation, real MLflow run evidence, registered model version, and persisted artifacts. | `Implemented` for baseline MLOps evidence | `src/realtyscope/ml/features.py`; `src/realtyscope/ml/train.py`; `docs/ml/phase5-non-leaky-model.md`; `docs/ml/phase6-mlflow-registration.md`; `tests/test_ml_features.py`; `tests/test_ml_training.py`; `services/mlflow/Dockerfile`; `services/trainer/Dockerfile`. | Phase 6 verified MLflow run `4999892d2d92402ab78e1209203c338e` and registered `realtyscope-price-model` version `3`. The model is still a baseline, not a final appraisal model. |
| Backend: FastAPI, `/data` endpoints, `/predict` endpoint | FastAPI app has `/health`, `/listings`, assignment-compatible `/data`, `/stats/data-quality`, `/predict`, `/model/metadata`, and `/monitoring/status`; Swagger is available through FastAPI. | `Partial` | `services/api/app/main.py`; `tests/test_api_health.py`; `tests/test_api_data_routes.py`; `tests/test_api_prediction_contract.py`; `tests/test_api_monitoring.py`. | `/data` is implemented as a schema-compatible alias for the Redis-backed listing read path. Prediction serving still depends on a model artifact being present in the shared model volume. |
| Frontend: Streamlit Dashboard, Plotly charts, Interactive Filters | Streamlit reads API data and displays tabbed sections for KPI overview, filtered/paginated listing preview, price distribution, median price by rooms, coordinate map with OpenStreetMap attribution, prediction, monitoring status, last successful collection metrics, recent errors, metrics, and feature importance. | `Partial: reviewer visuals, tabs, and last-success display implemented` | `services/streamlit/app.py`; `services/streamlit/api_client.py`; `services/streamlit/dashboard_charts.py`; `tests/test_streamlit_api_client.py`; `tests/test_streamlit_scaffold.py`; `tests/test_streamlit_dashboard_charts.py`. | Remaining frontend polish is conservative trend views if data supports them and final browser checks across useful viewport sizes. |
| Docker Compose one-command local environment | Compose defines `db`, `redis`, `mlflow`, `api`, and `streamlit` with healthchecks for db/redis/api/streamlit. | `Implemented` for local runtime | `docker-compose.yml`; `services/api/Dockerfile`; `services/streamlit/Dockerfile`; `services/mlflow/Dockerfile`; `docs/development/local-environment.md`; `docs/ml/phase6-mlflow-registration.md`. | Phase 6 verified `docker compose -p realtyscope up --build -d` directly from the Windows-mounted repo using scoped build contexts, without the old temp context workaround. |
...
| Cache | Redis. | Redis service plus API cache for `/listings` and `/data` payloads. | `Implemented` for API read path | `docker-compose.yml`; `.env.example`; `pyproject.toml`; `services/api/app/main.py`; `tests/test_api_data_routes.py`. | Cache is intentionally scoped to data preview reads; writes still go through PostgreSQL. |
...
| Experiment tracking | File logs required; MLflow advanced/grade-5. | MLflow service, trainer image, real run evidence, logged params/metrics/artifacts, and registered model version exist. | `Implemented` for baseline MLOps evidence | `src/realtyscope/ml/train.py`; `tests/test_ml_training.py`; `services/mlflow/Dockerfile`; `services/trainer/Dockerfile`; `docs/ml/phase6-mlflow-registration.md`; `pyproject.toml`. | Verified run `4999892d2d92402ab78e1209203c338e` is registered as `realtyscope-price-model` version `3`. |
| Backend API | FastAPI, Pydantic, uvicorn, Swagger. | FastAPI + Pydantic stack is tested with data, `/data` alias, prediction, model metadata, and monitoring endpoints. | `Partial` | `services/api/app/main.py`; `services/api/Dockerfile`; `tests/test_api_health.py`; `tests/test_api_data_routes.py`; `tests/test_api_prediction_contract.py`; `tests/test_api_monitoring.py`. | Prediction serving still requires the trained artifact to be available in the mounted model volume. |
| Frontend and visualization | Streamlit, Plotly, maps via `streamlit.map` or advanced mapping libs. | Streamlit dashboard includes overview, Data Explorer filters, reviewer charts/map, prediction, monitoring, and model-insight sections backed by FastAPI. | `Partial: Streamlit map/charts implemented` | `services/streamlit/app.py`; `services/streamlit/dashboard_charts.py`; `services/streamlit/Dockerfile`; `tests/test_streamlit_api_client.py`; `tests/test_streamlit_scaffold.py`; `tests/test_streamlit_dashboard_charts.py`. | Plotly/time-series views remain optional; any trend view should stay conservative until repeated observation semantics are revalidated. |
