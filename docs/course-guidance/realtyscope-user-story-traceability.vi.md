...
| US-08 | Data analyst muốn hiểu logic model để tin tưởng dự đoán. Tiêu chí: feature importance, mô tả features, version model, bonus SHAP charts. | Phase 5 document feature versions, leakage caveats, grouped validation metadata, model version, và metrics cho artifact v2. | Phase 5 / `Một phần: đã có metadata không leakage` | `docs/ml/phase5-non-leaky-model.vi.md`; `docs/ml/phase5-non-leaky-model.md`; `src/realtyscope/ml/features.py`; `src/realtyscope/ml/train.py`; `/predict` response trong `services/api/app/main.py`; Streamlit prediction scaffold trong `services/streamlit/app.py`. | Feature importance/SHAP và model insight UI cuối vẫn cần được harden trong slice API/Streamlit. |
