...
- `src/realtyscope/analysis/eda_summary.py` hiện có CLI EDA summary đọc database thật và xuất markdown tiếng Việt cùng JSON metrics. CLI này đã có test bằng database seed qua persistence path, nhưng Gate EDA vẫn chỉ được claim hoàn thành sau khi chạy trên row Domclick thật.
...
- Streamlit hiện đã có dashboard slice đầu tiên đọc từ các endpoint đó, hiển thị data-quality metrics và bảng listing preview. Dashboard này có test cho API client và app shell, nhưng Gate dashboard vẫn cần bằng chứng runtime với rows Domclick thật.
