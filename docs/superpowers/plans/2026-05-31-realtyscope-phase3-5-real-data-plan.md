...
- `src/realtyscope/analysis/eda_summary.py` now provides a DB-backed EDA summary CLI that writes Vietnamese markdown and JSON metrics from persisted database rows. It is tested with seeded persistence rows, but EDA conclusions still need to be generated from real Domclick rows before the EDA gate can be claimed complete.
