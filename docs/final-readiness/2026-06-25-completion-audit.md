...
## 2026-06-26 Current Docker Data Refresh And Monitoring Addendum

This addendum supersedes `17,046` as the freshest Docker database count. It does not supersede the selected-model training row count: the model remains the validated `random_forest` snapshot trained on `17,046` rows.

- Controlled Domclick refresh: after the scheduled task initially hit QRATOR, a Chrome/CDP preflight succeeded and a bounded 50-page capture wrote `data/raw/domclick/2026-06-26-bulk` with `1,000` records.
...
- Docker `/monitoring/status`: `17,287` listings, `source_counts={'cian': 2436, 'domclick': 14851}`, `45,764` observations, `23` observation dates, `last_observed_date=2026-06-26`, `listings_with_observation_history=7,956`, `listing_price_change_count=1,470`, and `inferred_lifecycle_target_rows=6,105`.
- OSM feature coverage after the refresh is `17,046 / 17,287` (`98.61%`), with provenance from local extract + prior live Overpass + exact-coordinate derivation. Do not describe this as `100.0%` coverage for the refreshed table.
- Model freshness is explicit: `model.data_freshness.status=validated_snapshot`, `model_rows_total=17,046`, `current_listings_total=17,287`, `row_delta=241`, `requires_retrain=false`. Do not claim automatic retrain or improved model quality from the new rows.
- Monitoring hardening: `/monitoring/status` exposes bounded `recent_logs` plus compatibility `recent_errors`; the Domclick scheduler wrapper can persist QRATOR/source-blocked pre-batch failures into `app_logs`.
- Scheduler target was corrected from the old `RealtyScope` repo to `RealtyScope-stitch-hybrid-redesign-20260623`; the previous Scheduled Task XML is backed up at `output/scheduler/realtyscope-domclick-scheduled-batch-before-20260626.xml`.
- Docker static audit printed `api 17287 {'cian': 2436, 'domclick': 14851}`. Docker CDP audit verified trend data through `2026-06-26`, selected candidate `random_forest`, and seven clean screenshots.
- WSL Compose smoke showed `api`, `streamlit`, `db`, and `redis` healthy, with `mlflow` up. API `/health` returned Docker `status=ok`, Streamlit health returned `ok`, and `/monitoring/status` returned `listings_total=17287`.
- Future full rebuild proof should also be recorded through WSL, because Windows PowerShell Docker CLI availability is not assumed on this workstation.
...
| Use real data only; do not fabricate counts or metrics | Latest Docker API reports `17,287` listings, `source_counts={'cian': 2436, 'domclick': 14851}`, `45,764` observations, and all audit payloads are `mode=api`. | Satisfied |
...
| Serve validated selected model with explicit freshness caveat | Docker `/model/metadata` reports `selected_candidate=random_forest`, `candidate_count=3`, `rows_total=17,046`, `train_rows=13,636`, `test_rows=3,410`, `r2=0.8653013476373554`, `mae=7,638,132.733793359`, `rmse=17,470,229.328815047`; Docker `/monitoring/status` reports `model.data_freshness.status=validated_snapshot`, `current_listings_total=17,287`, `row_delta=241`, and `requires_retrain=false`. | Satisfied with freshness caveat |
...
| OSM strategy is real, resumable, and honest | Latest Docker data quality reports `osm_features_total=17,046`, `osm_featured_listings=17,046`, `osm_coverage_pct=98.61` against `17,287` listings, `osm_local_extract_rows=4,487`, `osm_live_rows=16`, `osm_coordinate_derived_rows=12,543`, and `osm_infrastructure_coverage_source=local_extract+live_overpass+coordinate_exact_match`. | Satisfied with refreshed-coverage caveat |
...
| Do not claim OSM-backed clustering unless coverage is real | Current persisted OSM feature coverage is `17,046 / 17,287` (`98.61%`) with explicit provenance. Do not describe it as all-live Overpass coverage or as `100.0%` for the refreshed table. | Satisfied with caveat |
...
| Dashboard/Data count consistency | Latest static audit and CDP report `listings_total=17,287`, `data_count_source=api`, `source_counts={'cian': 2436, 'domclick': 14851}`, and trend data through `2026-06-26`. | Satisfied |
