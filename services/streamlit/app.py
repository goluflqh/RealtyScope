...
st.caption("Phase 7 course-readiness dashboard")

row_limit = st.sidebar.selectbox("Rows", [25, 100, 500, 1000], index=3)
listing_page = st.sidebar.number_input("Page", min_value=1, value=1, step=1)
listing_offset = (listing_page - 1) * row_limit
...
(
    ...
    prediction_tab,
    monitoring_tab,
) = st.tabs(["Overview", "Data Explorer", "Visuals", "Prediction", "Monitoring & Model"])
...
with data_explorer_tab:
    ...
    if data.listings:
        ...
        first_row = listing_offset + 1
        last_row = listing_offset + len(data.listings)
        st.caption(f"Showing rows {first_row}-{last_row} of {total} listings")
...
with prediction_tab:
    ...
    with st.form("baseline-prediction-form"):
        ...
        with input_columns[1]:
            ...
            observation_count = st.number_input(
                "observation_count", min_value=0.0, value=1.0, step=1.0
            )
